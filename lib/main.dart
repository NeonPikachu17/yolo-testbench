...
class _VisionScreenState extends State<VisionScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // **IMPROVEMENT 1: Constrain the main content width for better large-screen layout.**
      // This centers the content and gives it a max width, preventing it from stretching
      // uncomfortably wide on tablets or desktops.
      body: Center(
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      if (constraints.maxWidth > 700) {
        return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // **IMPROVEMENT 3: Constrain the results list width.**
          Expanded(
            flex: 4, 
            child: ConstrainedBox(
  ...
  /// This now uses a LayoutBuilder to switch between a single-column layout on
  /// narrow screens and a two-column layout on wider screens.
...
class _DetectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < recognitions.length; i++) {
      ...
      // Scaled coordinates for the current bounding box on the canvas
      ...
      // 1. DRAW MASK WITHIN BOUNDING BOX (NEW LOGIC)
      if (showMasks && maskImage != null) {
      // if (showMasks && maskImage != null && isSelected) {
          // Save the canvas state before clipping
          canvas.save();
          
          // Clip the canvas to the area of the current bounding box
          canvas.clipRect(boundingBoxRect);

          // Paint the full mask image, but only the part that intersects the clipped area will be drawn.
          final maskPaint = Paint()
            ..colorFilter = ColorFilter.mode(color.withOpacity(maskOpacity), BlendMode.srcIn); // Use the class color for the mask
          
          // This assumes the maskImage is the same size as the original image was when inference was run.
          // We draw the mask across the whole image space on the canvas
          canvas.drawImageRect(
            maskImage!,
            Rect.fromLTWH(0, 0, maskImage!.width.toDouble(), maskImage!.height.toDouble()), // Source: Full mask image
            Rect.fromLTWH(0, 0, size.width, size.height), // Destination: Full canvas size
            maskPaint,
          );
